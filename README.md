...
Работает сам, без ключей и оплаты. Бот использует бесплатные GitHub Models (`openai/gpt-4.1-mini`) через встроенный токен репозитория. У каждой новости будут:
- русский заголовок;
- 2–4 предложения о том, что произошло: детали и цифры из текстов всех изданий, где вышла новость, плюс описание самой статьи;
- **Почему важно:** как событие может повлиять на рынки;
- затронутые активы и направление: 🟢 позитив, 🔴 негатив или 🟡 неоднозначно.

В начале дайджеста будет **Общий фон** рынка. Факты ИИ берёт только из текстов источников, а какие новости попадут в дайджест, по-прежнему решают фильтры.

Если GitHub Models недоступны, бот переведёт заголовки и описания через Google Translate. Можно подключить и свой ключ: секрет `LLM_API_KEY`, переменные `LLM_BASE_URL` и `LLM_MODEL`, подходит OpenRouter или любой OpenAI-совместимый API.
