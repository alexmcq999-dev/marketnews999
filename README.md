...
Какие модели. Сначала бот пробует бесплатные (Llama). Их лимит — 50 запросов в день, боту нужно около 10. Если бесплатные перегружены, бот переключается на `openai/gpt-4o-mini`, но только при наличии кредитов на счёте. Этот запасной вариант стоит меньше цента в день, пополнить можно и криптой, а заодно вырастет лимит бесплатных моделей. Выбрать конкретную модель можно переменной `LLM_MODEL`.
